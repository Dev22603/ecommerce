...
import { cartService } from "../services/cartService"; // Updated import
...
const ProductCard = ({ product }) => {
    ...
    useEffect(() => {
        const fetchCartItem = async () => {
            ...
            try {
                ...
                const cartData = await cartService.checkCartItemQuantity(
                    product.id,
                    token
                ); // Updated call
    ...
    const handleAddToCart = async () => {
        ...
        if (user.role === "admin") {
            return toast.error(
                ...
                {
                    autoClose:1200, // Toast duration: 1.5 seconds
                }
            );
        }
        ...
        try {
            ...
            if (cartItem) {
                ...
                await cartService.updateCart(product.id, quantity + 1, token); // Updated call
    ...
    const handleUpdateQuantity = async (newQuantity) => {
        ...
        try {
            setIsLoading(true);
            await cartService.updateCart(product.id, newQuantity, token); // Updated call
    ...
    const handleRemoveFromCart = async () => {
        ...
        try {
            setIsLoading(true);
            await cartService.removeItemFromCart(product.id, token); // Updated call
