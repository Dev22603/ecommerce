...
export const cartService = {
	async addItemToCart(userId: number, data: unknown) {
		try {
			...
			if (!(error instanceof ApiError)) {
				logger.error("Add item to cart failed", { userId, productId: (data as any)?.product_id, error: (error as Error).message, stack: (error as Error).stack });
			}
			throw error;
		}
	},
	...
	async updateCart(userId: number, data: unknown) {
		try {
			...
			if (!(error instanceof ApiError)) {
				logger.error("Update cart failed", { userId, productId: (data as any)?.product_id, error: (error as Error).message, stack: (error as Error).stack });
			}
			throw error;
		}
	},
