  ...
  async createFromCart(userId: number, addressId: number) {
    return await prisma.$transaction(async (tx) => {
      ...
      const totalAmount = cartItems.reduce(
        (sum, item) => sum + item.quantity * item.product.salesPrice,
        0,
      );
      ...
      // ⚡ Bolt Optimization: Use Promise.all instead of sequential awaits in a loop
      // This prevents event loop blocking and allows database updates to execute concurrently
      // Impact: Reduces latency of order creation, especially for large carts (O(1) vs O(N) database roundtrips)
      await Promise.all(
        cartItems.map((item) =>
          tx.product.update({
