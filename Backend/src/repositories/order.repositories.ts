...
export const orderRepository = {
  async createFromCart(userId: number, addressId: number) {
    return await prisma.$transaction(async (tx) => {
      ...
      // ⚡ Bolt: Execute stock updates concurrently to prevent event loop blocking
      // and significantly speed up the transaction execution when multiple items are in the cart.
      await Promise.all(
        cartItems.map((item) =>
          tx.product.update({
