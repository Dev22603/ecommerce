...
export const orderRepository = {
	async createFromCart(userId: number, addressId: number) {
		return await prisma.$transaction(async (tx) => {
			...
			// ⚡ Bolt: Use Promise.all to execute independent database updates concurrently
			// Impact: Prevents event loop blocking within the Prisma transaction and reduces total latency
			await Promise.all(
				cartItems.map((item) =>
					tx.product.update({
