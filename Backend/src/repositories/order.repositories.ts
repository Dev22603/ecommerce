...
export const orderRepository = {
  async createFromCart(userId: number, addressId: number) {
    return await prisma.$transaction(async (tx) => {
      ...
      // ⚡ Bolt Optimization: Use Promise.all instead of sequential awaits in loop
      // This prevents event loop blocking and allows independent database updates to run concurrently
      await Promise.all(
        cartItems.map((item) =>
          tx.product.update({
