...
export const orderRepository = {
  async createFromCart(userId: number, addressId: number) {
    return await prisma.$transaction(async (tx) => {
      ...
      // Optimization: Use await Promise.all(...) instead of sequential await inside loop
      // within Prisma transaction to execute independent database updates concurrently
