...
export const orderRepository = {
  async createFromCart(userId: number, addressId: number) {
    return await prisma.$transaction(async (tx) => {
      ...
      // ⚡ Bolt Performance Optimization: Execute independent database updates concurrently
      // using Promise.all to prevent event loop blocking and reduce network roundtrip time
      await Promise.all(
        cartItems.map((item) =>
          tx.product.update({
